<template>
  <view class="container">
    ...
    <!-- 	<nav-bar title="交易设置" backState="1000" bgColor="#0278FF" fontColor="#ffffff">
			<view slot="right">
				<view class="isOkDoit" @click="comfirmSet">确认</view>
			</view>
		</nav-bar> -->
    ...
    <view class="setting-content">
      <view class="contnet-top">
        ...
        <view class="contnet-top-list hairline--bottom" v-show="tradingOn">
          ...
          <view class="top-list-left-right">
            ...
            <view class="top-list-right-title">{{tradingOn}}</view>
          </view>
        </view>
        ...
        <view class="contnet-top-list hairline--bottom">
          <view class="top-list-left">
            <!-- <view class="img-box">
							<image style="width: 70upx;height: 70upx;"
								src="../../static/image/index/icon01.png" mode=""></image>
						</view> -->
            <view class="top-list-left-title">首单金额(USDT)</view>
          </view>
          <view class="top-list-left-right">
            <view class="top-list-right-num">
              <input v-model="tradSetting.firstOrder" type="number" />
            </view>
        ...
        <view class="contnet-top-list hairline--bottom">
          <view class="top-list-left">
            <!-- 	<view class="img-box">
							<image style="width: 70upx;height: 70upx;"
								src="../../static/image/index/jiabei.png" mode=""></image>
						</view> -->
        ...
        <view class="contnet-top">
          <view class="contnet-top-list hairline--bottom">
            ...
            <view class="top-list-left-right">
              ...
              <!-- <view class="top-list-right-title">单</view> -->
            </view>
          </view>
          ...
          <view class="contnet-top-list hairline--bottom">
            <view class="top-list-left">
              <!-- <view class="img-box">
								<image style="width: 70upx;height: 70upx;"
									src="../../static/image/index/icon03.png" mode=""></image>
							</view> -->
          ...
          <view class="contnet-top-list hairline--bottom" @tap="screenClick">
            ...
            <view class="top-list-left-right">
              <view class="top-list-right-text">阶梯补仓</view>
              <view class="top-list-right-title">
                <image class="arror" :class="{rotate: isDecline}" src="../../static/images/icon_cl_more@3x.png"
          ...
          <!-- 阶梯补仓 -->
          <view class="ladderCovering" v-show="isDecline">
            <view class="contnet-top-list hairline--bottom" v-for="item in tradSetting.buyInDecline">
              <view class="top-list-left">
                <view class="top-list-left-title sub">{{item.title}}</view>
              </view>
              <view class="top-list-left-right">
                <view class="top-list-right-num">
                  <input v-model="item.decline" type="number" />
                </view>
                <view class="top-list-right-title">%</view>
              </view>
            </view>
          ...
          <view class="contnet-top-list hairline--bottom">
            ...
            <view class="top-list-left-right">
              ...
              <view class="strategy_item" :class="{active: strategyIndex === 0}" @click="switchStrategy(0)">循环策略</view>
              <view class="strategy_item" :class="{active: strategyIndex === 1}" @click="switchStrategy(1)">单次策略</view>
    ...
    <view class="bottomWrap">
      <view class="btn_save" @click="comfirmSet(0)">保存</view>
      <view class="btn_start" @click="comfirmSet(1)">保存并启动</view>
    </view>
...
<script>
  export default {
    data() {
      return {
        isDecline: false, // 是否展示阶梯补仓
        strategyIndex: 0, // 0: 循环策略 1: 单次策略
        tradingOn: '',
    ...
    onLoad(option) {
      // this.bourseId = this.$store.state.userInfo.bourseId;
      this.strategyId = option.status;
      this.bourseId = option.bourseId;

      // todo: 交易对id改为从缓存中取
      
      if(option.tradingOnId) {
        this.tradingOnId = option.tradingOnId;
      }
      
      
      if (option.tradingOnId) {
        this.tradingOnIds = [option.tradingOnId];
      } else {
        this.tradingOnIds = JSON.parse(this.$common.getStorage('ids'))
      }

      if (option.status) {
        this.tradingOn = option.status
      }

      if (option.tradingOn) {
        this.tradingOn = option.tradingOn.replace(/-/, '/');
      }

      // this.tradingOn = option.tradingOn.replace(/-/,'/');

      console.log('this.bourseId', this.bourseId);
      console.log('this.strategyId', option.status);
      console.log('this.tradingOnId', option.tradingOnId);

      // this.tradingOnIds = this.$common.getStorage('ids')
      // this.tradingOnId = JSON.parse(this.tradingOnIds)[0]
      // console.log(JSON.parse(this.tradingOnIds)[0])
    ...
    methods: {
      ...
      /**
       * 点击确认进行设置
       * @param {number} startType 保存后是否启动 1：启动 0：不启动
       */
      comfirmSet(startType) {
        console.log(this.tradSetting)
        let _this = this;
        ...
        _this.$api.setUpMany({
          ...
          tradingOnIds: _this.tradingOnIds,
          ...
          tradingOnType: _this.strategyIndex,
          enabled: startType
        }, res => {
          console.log(res)
          if (res.code == 200) {
            setTimeout(() => {
              ...
              setTimeout(() => {
                uni.navigateBack(-1);
                // _this.$common.skipTo('/pages/trade');
              }, 500)
