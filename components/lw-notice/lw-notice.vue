  ...
  <view>
    <view class="contentBox" v-if="list.length" @click="toEmit()"
      ...
      <view v-if="showIcon" class="supBox">
        ...
        <image src="../../static/images/index/home_gonggao@3x.png" mode="aspectFit"></image>
      </view>

      <view v-if="list.length > 1" class="content">
        <view v-for="(item, index) in list" :key="index">
          <view class="loopItemBase" :class="index==0&&firstIn?'fistInClass':''" :animation="realAnimation(index)"
            v-if="aindexArr.includes(index)" :style="'line-height:'+height+'rpx;'">
            {{ item }}
          </view>
        </view>
      </view>
  ...
  .contentBox {
    ...
    .offBox {
      ...
      .icon_more {
        width: 24rpx;
        height: 24rpx;
        background: url("../../static/images/index/home_more@3x.png") no-repeat;
