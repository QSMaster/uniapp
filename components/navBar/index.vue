			...
			<view class="header_content">
				<view class="header_left_box">
					<slot name="left">
						<view class="header_left_info"
							...
							<view class="header_left_back" :class="[ isTwoBtn ? 'header_btnMongol_left_back' : '' ]"
								...
								<image class="header_icon" v-if="isWhite" src="/static/images/icon_bar_back@3x.png"
									mode="aspectFit"></image>
								<image class="header_icon" v-else src="/static/images/icon_bar_back@3x.png"
									mode="aspectFit"></image>
							</view>
							...
							<view class="header_left_home" :class="[isTwoBtn ? 'header_btnMongol_left_home' : '']"
								...
								<image class="header_icon" v-if="isWhite" src="/static/images/icon_bar_back@3x.png"
									mode="aspectFit"></image>
								<image class="header_icon" v-else src="/static/images/icon_home_white.png"
									mode="aspectFit"></image>
							</view>
						</view>
					</slot>
					<view class="header_title" v-if="!titleCenter && ($slots.default || navTitle)"
						...
						<slot>
							<text :style="{ color: navFontColor }"><span
									style="font-size: 32upx;">{{ navTitle }}</span></text>
						</slot>
					</view>
				</view>
				...
				<view class="header_left_box">
					<slot name="transparentFixedLeft">
						<view class="header_left_info header_transparentFixed_left_info"
							...
							<view class="header_left_back" :class="[ isTwoBtn ? 'header_btnMongol_left_back' : '' ]"
								v-if="back && !firstPage" @click="onBackPage">
								<image class="header_icon" v-if="isWhite" src="/static/image/icon_back_white.png"
									mode="aspectFit"></image>
								<image class="header_icon" v-else src="/static/image/icon_back_white.png"
									mode="aspectFit"></image>
							</view>
							<text class="header_left_line" v-if="isTwoBtn"></text>
							<view class="header_left_home" :class="[isTwoBtn ? 'header_btnMongol_left_home' : '']"
								v-if="(firstPage && back) || home" @click="onBackHome">
								<image class="header_icon" v-if="isWhite" src="/static/image/icon_home_white.png"
									mode="aspectFit"></image>
								<image class="header_icon" v-else src="/static/image/icon_home_white.png"
									mode="aspectFit"></image>
							</view>
						</view>
					</slot>
					<view class="header_title" v-if="!titleCenter && (navTitle || $slots.transparentFixed)"
						...
						<slot name="transparentFixed">
							<text :style="{ color: navTransparentFixedFontColor }"><span
									style="font-size: 32upx;">{{ navTitle }}</span></text>
						</slot>
					</view>
				</view>
...
<script>
	...
	export default {
        ...
        console.log('设置手机状态栏颜色');
          ...
          console.log('frontColor', frontColor);
...
<style lang="scss">
	...
	.header_shadow {
		...
		// border-width: 2rpx;
		// border-color: #f5f5f5;
		box-shadow: 0rpx 0rpx 28rpx 0rpx rgba(227,228,230,0.50)
